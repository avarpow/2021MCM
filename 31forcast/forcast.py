...
ddata = data[u'pro']
# 检验
# ts_ap = tsplot(ddata, title='A Given Training Series',
#                lags=3)  # ACF 和 PACF 检验
...
model = ARIMA(ddata, order=(2, 0, 0))  # 第二个参数代表使用了二阶差分
results_AR = model.fit(disp=-1)
plt.plot(ddata)
plt.plot(results_AR.fittedvalues, color='red')  # 红色线代表预测值
plt.title('RSS:%.4f' % sum((results_AR.fittedvalues-ddata)**2))
plt.show()
forecast_n = 120  # 预测未来12个天走势
